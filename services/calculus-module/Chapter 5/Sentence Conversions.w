...
- If the NP talks about a single definite thing $C$, then $t=C$ and $\phi = T$,
the empty proposition.
- If the NP talks about a single definite thing $C$ but imposes conditions about its current
...
does not use $v$.
- If the NP talks about a single but vague thing, identifying it only by its
current situation, then $t=v$ for some variable $v$ and $\phi$ is a proposition having
$v$ as its unique free variable.
- If the NP talks about a range, number or proportion of things, then $t=v$
...
- $\Sigma'$ remains a syntactically correct proposition with well-formed
quantifiers,
- $\Sigma'$ has the same number of free variables as $\Sigma$, and
- in all situations and for all possible values of any free variables,
