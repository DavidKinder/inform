...
When writing code in InC (the slight extension of C granted by inweb), this
can actually be written as a function call:
= (text as C)
	if (<competitor>(W)) ...
=
...
- |...| matches any non-empty text, as shown above.
- |***| matches any text, including possibly the empty text.
- |......| matches any non-empty text in which brackets are used in a
balanced way -- thus they would match "alpha beta gamma" or "alpha (the
Greek letter)", but not "alpha (the" or "Greek letter)".
- |###| matches any single word, counting words as the lexer does.
